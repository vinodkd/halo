...
exports.run = function(ptree){
	console.log(ptree.value.content[0].alias);
	var seqNode = ptree.value.content[0];
...
function checkSemantics (seqNode) {
	...
	revMap = {};
	for(var n=0; n< seqNode.value.content.length; n++){
		var node = seqNode.value.content[n];
		node.out = [];
		// this handles the "natural graph nature" of sequences 
		// by adding the next node in the out list for the previous node.
		if(n>0) seqNode.value.content[n-1].out.push(node);
		var type = node.type;
...
function moveTo (node,pos,parent) {
	...
	delete parent.value.content[pos];
}

function checkIf (node,index) {
	if(node.name != 'if') throw new Error("if is the only allowed subgraph in ssi");
	var ifNode = node.value.content;
	...
	if(ifNode[1].targets[0].opr != 'rel') throw new Error("if true edge missing");
	ifNode[0].out.push(ifNode[1].targets[0]);

	if(ifNode[1].targets[1].opr != 'rel') throw new Error("if false edge missing");
...
function nextStep (seqNode,step,ret) {
	// start
	if(step == null){
		return seqNode.value.content[0];
	}
	if(ret < 0){	// error
		return null;
	}
	if(step.name == "if"){
		...
		// console.log("ret:" + seqNode.value.content[revMap[ret.dest]]);

		return seqNode.value.content[revMap[ret.dest]];
	}
	return step.out[0];
}
...
function execute (step) {
	...
	if(step.name == "if"){
		...
		var ret = baseExecute(step.value.content[0].name);
		// console.log("op:"+ret.output + "true:"+(ret.output.trim() == "1"));
		return ret.output.trim() == "1" ? step.value.content[0].out[0] : step.value.content[0].out[1];
	}
